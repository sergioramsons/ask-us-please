...
-- Create ticket comments for these customer replies
INSERT INTO ticket_comments (ticket_id, comment, is_internal, created_by, email_id, created_at)
VALUES 
(
...
-- Create an index on incoming_emails subject column for better performance
CREATE INDEX IF NOT EXISTS idx_incoming_emails_subject ON incoming_emails(subject);

-- Create an index on incoming_emails from_email for better performance  
